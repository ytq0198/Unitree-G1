...
8. 官方评测环境向 actor 的第 96-97 列提供未缩放的机体坐标 waypoint 位移；`forward_yaw` 训练环境则在同两列提供 `[forward_speed, yaw_rate]`。因此提交包的 `model.py` 必须在策略归一化之前执行与训练端完全相同的转换，并保持其余 283 列不变。该等价性由回归测试覆盖，训练、评测、视频和导出共用 `max_command_speed` 与 `min_turning_speed`。
