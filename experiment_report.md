...
6. 已部署一次性 H5 资源守候脚本：H5 是 64 env 的单卡训练，不要求四卡同时空闲。实测 GPU 0 尚有约 36 GB 可用显存，因此条件调整为可用显存超过 24 GB、瞬时利用率低于 20% 时启动；允许与 WoTE 共享显存和算力，但不跨卡做分布式 PPO。
7. 2026-08-12 20:51，H5 已选择 GPU 0 启动。启动后可用显存仍为约 36 GB，未发生 OOM；进程暂处于共享磁盘文件页读取等待，累计读取量持续增长但尚未建立 CUDA 上下文，保留后台进程继续加载。
