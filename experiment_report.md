# Course Project 实验完成报告

## 1. 当前状态

**Height 接口 baseline 已完成训练、评估和提交导出，但当前策略尚不能正常完成复杂地形导航；继续优化 locomotion/navigation 是首要任务。**

- 本地代码：`D:\暑期综合实践活动\暑期综合实践活动II\labs\course project\course_project\course_project`
- 服务器工作区：`/mnt/localDisk3/weizian/unitree-g1-work`
- Conda：`/mnt/localDisk3/weizian/conda_envs/summer`
- GPU：4 x NVIDIA RTX A6000；完整场景单进程稳定上限为 64 env
- GitHub：<https://github.com/ytq0198/Unitree-G1>

服务器端只修改 `/mnt/localDisk3/weizian` 内的项目和 Conda 环境，不修改 Lab4、Lab7 或其他实验目录。

## 2. 已完成实现

- 独立实现 83 维 AMP state、深度归一化、yaw 坐标变换、导航奖励和平滑惩罚。
- 构建顺序 waypoint command、随机路线、路线推进/成功指标和 29 维直接关节控制环境。
- 支持 Height/Depth、训练、评估、视频和严格三文件提交导出。
- 支持 Lab7 checkpoint 迁移、可配置网络宽度、低噪声 warm start、朝向课程、命令速度和步态保持约束。
- 评估额外报告存活步数、跌倒率和 timeout rate，防止仅按训练奖励挑选模型。
- 本地测试 10/10 通过，关键 Python 文件通过语法检查。
- Depth smoke 已通过：2 env、16 steps、29 维动作、83 维 AMP state，深度张量为 `[2,1,60,80]`。

## 3. 实验记录

| ID | 主要配置 | 结论 |
|---|---|---|
| H0 | 随机初始化，64 env，1000 iter | 学会站立但不导航，progress 约 0.29% |
| H1-H4 | Lab7 warm start、修正 dt、0.6 m/s 局部命令 | 最佳 3 起点 progress 约 1.8%，但约 1 秒后跌倒 |
| H5 | waypoint velocity tracking weight=2 | 速度跟踪过强，独立评估约 0.8%，不如 H4 |
| H6-H7 | `forward_yaw` 命令与不同学习率 | 预训练策略基本忽略 yaw，训练快速退化，停止该路线 |
| H8 | 大网络 PPO warm start、aligned heading | 原 checkpoint 探索 std 约 0.5，微调时破坏步态 |
| H9 | aligned heading，std=0.1，lr=1e-5 | 建立当前最佳导航模型；随机朝向 3 起点约 1.50% |
| H10 | 从 H9 扩大初始朝向范围 | progress 降低，说明朝向课程跨度过快 |
| H11 | 随机朝向、关闭训练推扰、std=0.05/0.1 | 最佳约 1.16%，未超过 H9 |
| H12 | 恢复兼容的 Lab7 物理约束，速度 0.3/0.4 | 存活最多提高到 352 步，但 3 起点最高约 1.37% |
| H13 | `5e-6` 学习率，初始朝向范围 `0.30-0.60 rad` | 50 次更新后仍退化，说明仅缩小课程跨度不能阻止步态遗忘 |
| H14 | 冻结 actor，仅训练第一层 waypoint 两列 | 保持网络结构兼容；随机朝向、导航权重 2 的正式 progress 为 1.740% |
| H15 | H9 与 H14 waypoint 权重插值 | 75% 适配量取得当前最佳 1.972%，并显著延长存活 |

## 4. 正式 10 起点评估

统一使用 10 个不重复随机起点，每次最多 5000 步。

| 候选模型 | Route progress | Success | 平均步数 | 跌倒率 | Smoothness |
|---|---:|---:|---:|---:|---:|
| H9 `model_199.pt` | **0.017691** | 0 | **554.2** | 1.0 | 0.23550 |
| H12 gait=1.0 `model_100.pt` | 0.016022 | 0 | 172.1 | 1.0 | 0.21476 |
| H12 gait=0.5 `model_150.pt` | 0.014884 | 0 | 156.2 | 1.0 | 0.22368 |
| H12 稳定候选 `model_50.pt` | 0.013601 | 0 | 239.3 | 1.0 | **0.18558** |
| H14 waypoint-only `model_100.pt` | 0.017398 | 0 | 165.2 | 1.0 | 0.26902 |
| H15 waypoint blend 75% | **0.019724** | 0 | **1297.7** | **0.8** | **0.19691** |

H15 的逐起点诊断显示，聚合均值不能理解为正常行走：8/10 起点在 63-289 步内跌倒，最大位移仅约 0.6-2.0 m；2/10 起点达到 5000 步 timeout，其中 seed 103 仅移动 0.22 m，属于站立而非导航。当前成功率仍为 0，策略不能视为完成复杂地形穿越。

当前最佳 checkpoint（H15）：

```text
/mnt/localDisk3/weizian/unitree-g1-work/outputs/warmstart/h15_blends/
h9_h14m199_a75.pt
```

## 5. 提交验证

已从 H9 最佳模型生成：

```text
outputs/submission/policy.pt
outputs/submission/model.py
outputs/submission/student.py
```

`policy.pt` 可加载，并对 `[2,285]` 输入产生全为有限数值的 `[2,29]` 输出。该行为与 Lab7 官方导出适配器一致；环境的 joint-position action term 负责后续动作缩放。导出时使用与 checkpoint 一致的 `512-256-128` 网络，已修复旧入口默认按小网络重建的问题。

旧版 300 帧视频使用自动 reset wrapper，摔倒后会在同一视频中开始新 episode，因此不能作为连续穿越证据。现已修复为首次终止即停止录像；后续报告只使用单 episode 视频，并同时报告路线推进和终止原因。

## 6. 已验证结论

1. Lab7 warm start 有效，但只有同时对齐输入布局、命令语义、网络结构、学习率和探索噪声才不会破坏步态。
2. aligned-heading 证明预训练策略能沿前向 waypoint 推进；随机朝向与侧向控制是当前主要瓶颈。
3. 强速度跟踪、直接改为 `forward_yaw`、过快扩大朝向范围均无效。
4. 恢复物理稳定约束可延长部分模型存活时间，但目前不足以提高正式路线分。
5. 最后 checkpoint 不一定最佳，必须采用多 checkpoint、独立随机起点评估。
6. 仅更新 waypoint 输入列可避免其他 actor 参数漂移；与原策略做 75% 插值后，正式 route progress 相对 H9 提高约 11.5%，且 20% 起点达到 5000 步 timeout。

课程 notebook 指向的 `grading_toolkit/grade.py` 当前既不在本地课程目录，也未在服务器 `/mnt/localDisk3/weizian` 下找到，因此暂时无法运行官方评分器；现有评估严格复用 notebook 规定的 10 个随机起点协议。

## 7. 下一步

- 暂停 Depth，先重训 Height 基础步态并通过 6 m 粗糙地形硬门槛。
- L0 使用 512 env、4 seeds 平地预训练；seed 4101/4103 的后期 checkpoint 已达到 20 s 无早停、速度误差约 0.016-0.020 m/s，平地 walking success 为 37.5%。
- 平地模型直接迁移完整粗糙地形仍约 1 s 跌倒，因此进入 L1 温和粗糙课程，不允许跳级。
- Height 导航只有在首 waypoint 到达、route success 非零且真实单 episode 视频验证后才视为达标。

## 8. 2026-08-13 算法审计与修复

在继续堆叠训练迭代前，对命令、奖励、终止、重置、AMP、迁移和评测链路进行了逐项审计。

确认并修复了三个会直接影响结论的问题：

1. 原导航稠密奖励比较相邻时刻“到当前 waypoint 的距离”。waypoint 更新后，旧航点近距离会与新航点远距离相减，产生错误的大负奖励。现改为沿当前折线路段投影的连续路径位置差。
2. waypoint 是 yaw-local 二维命令，但原速度跟踪使用包含 roll/pitch 的完整 body-frame 速度。现统一为 yaw-local 线速度，保证坡地上的坐标语义一致。
3. Lab7 粗糙阶段含世界 +X 推进奖励，同时初始 yaw 在 `[-pi, pi]` 随机，和 body-frame 前进命令冲突；评测也用世界 +X 位移却未固定 yaw。现训练推进奖励改为沿命令方向，固定穿越评测将初始 yaw 设为 0。

验证结果：

- 本地 12/12 单元测试通过；新增直线段投影与 90 度转弯连续性测试。
- 服务器 Height smoke 通过；实际 actor/critic 维度分别为 285/297，Lab7 为 286/298，warm-start 删除列正确。
- AMP terminal state、timeout bootstrap 和 50 Hz 相邻帧时序正确。seed 5103 的判别器损失由 7.456 收敛到 0.00250，梯度惩罚由 0.639 降至 0.0000369，暂无失衡证据。
- 修正初始朝向后，平地 seed 4101 `model_799.pt` 在 32 环境达到 100% traversal/walking success、20 s 无提前终止，线速度误差 0.0159 m/s。此前 37.5% 是评测坐标错误，不是步态失败。
- 同一模型直接迁移完整粗糙地形仅有 10.4% 平均推进、91.3% 提前终止，因此仍需温和粗糙地形课程训练。

Course Project 评测进一步增加 `waypoints_reached` 与
`first_waypoint_success`，用于区分首段局部推进、到达首航点和完整路线成功；
正式选模仍以未见随机场景为准。

`forward_yaw` 导航中的转弯最小前进速度由 0.4 m/s 修正为 0.1 m/s，并同步
到提交端观测适配器。原设置会在目标位于侧方或后方时强制机器人高速前冲，
与 yaw-rate 转向目标冲突。

### H16-H17：回到 Course Project 的导航训练

- H16 使用二维 `[vx, vy]` waypoint 命令，从修正后的粗糙步态初始化；3 个未见
  随机场景的 route progress 仅为 1.85%-2.17%，说明底层步态改善不会自动解决
  Course Project 的方向控制。
- H17 改用由 body-frame waypoint 生成的 `[forward, yaw_rate]` 命令，复用
  Lab7 actor 中前向和转向输入语义，但训练和评价均为 Course Project 环境。
- H17 最佳 `model_299.pt` 在 3 起点筛选中达到 13.74% route progress，
  平均存活 1046 步；在 10 个未见随机场景正式评测中达到 10.18% route
  progress、平均存活 789 步。首航点成功率仍为 0，因此尚未达到 Height 门槛。
- 当前主要泛化问题是每次训练的 64 个并行环境复制同一个 scene seed。下一阶段
  从 H17 checkpoint 跨多个不同场景连续微调，并逐步扩大初始朝向，而不是继续
  优化 Lab7 指标。

### H18-H20：失败分析

- H18 在四个不同 Course Project 场景上分别微调，最佳分支仅维持约 13.5%
  的 3 起点 route progress，说明单场景串行微调存在遗忘。
- H19 在 Course Project 三类地形上降低几何难度训练，但回到正式难度后仅推进
  约 3.9 m，不继续采用。
- H20 加入只针对 `fell_over` 的终止惩罚，`0/-50/-100/-200` 四档均未超过
  H17；失败不是单独缺少跌倒代价造成的。
- H17 到 H20 的 actor 归一化均值相对变化约 9.3%，critic 方差约 63.7%，
  而 actor 权重仅变化约 1%-1.8%。下一轮冻结 actor/critic 归一化，并对比
  全 actor 微调与只更新导航输入列，以保留复杂地形步态。
- H21 表明冻结归一化的全 actor 微调可保持约 6.19 m 推进，不冻结时退化到
  3.97 m；仅导航输入列不足以适应跨 tile 动力学。后续固定冻结归一化，并用
  训练专用起点偏移提高 platform-gap / stairs 边界的有效采样率。
- H22 的固定偏移训练在自身局部目标上改善，但原始中心起点下降到约 4 m；
  actor 与 H17 以 50% 融合曾在一次 3 起点筛选中到达首航点，但 10 起点复核
  未复现。后续不再用单次小样本认定突破，改用中心到边界的混合起点训练和
  重复 rollout 复核。

### H23-H25：地形接口审计与修复

- H23 在四个场景使用首段 `0-4.5 m` 混合起点训练，6 场景复评的最好分支平均
  推进 5.42 m，首航点到达率仍为 0，失败继续集中在约 5 m 的 tile 边界之后。
- 对照 Lab4 课程提供的 terrain-generator 后发现，原 `platform_gap` 被实现为
  跨越整块 tile 的 6 条平行沟槽，而课程定义是中心平台周围的一圈窄沟；原
  `pyramid_stairs` 只沿世界 x 方向变化，路线沿 y 方向进入时会撞上最高平台侧壁。
- 已将沟槽修正为四向可进入的中心平台环沟，将台阶修正为四向同心矩形环；满难度
  参数与课程工具对齐为 0.25 m 沟宽、0.08 m 阶高。第一版重叠盒体曾造成
  MuJoCo-Warp `nconmax overflow`，随后改为互不重叠的矩形环并通过 6 场景复评。
- 不重新训练时，H17 在修正地形上的 6 场景平均推进由约 5.37 m 提高到 6.31 m，
  证明此前失败包含地形实现错误，而不只是 PPO 训练不足。
- H25 在修正地形上微调 500 次，末期 checkpoint 全部退化；早期 checkpoint 在
  3 场景可推进约 8.0 m，但仍在距首航点约 2 m 的环沟外沿失败。速度扫描显示
  `0.35/0.45 m/s` 均约 6.3 m，`0.55/0.65 m/s` 更差。下一阶段采用沟宽和阶高
  从易到难的专项课程，而不是增加冲刺速度。

### H26-H40：沟槽与台阶分技能课程

- H26 在最低难度首次取得可复现突破：最佳 scene23 分支在 6 个未见场景平均推进
  15.27 m，首航点成功率 50%，平均到达 0.83 个航点。scene31 专家在低台阶场景
  能走满 5000 步、通过 3 个航点并推进 38.9 m。
- H27-H29 发现 PPO 的 `adaptive` 调度会把名义 `5e-7` 学习率放大到最高
  `4.32e-5`（约 86 倍），造成步态快速遗忘。已增加 `fixed/adaptive` 调度选项，
  后续课程使用固定学习率和短训练，并保留每阶段回退 checkpoint。
- 原难度 0 的沟宽仍为 0.08 m，不是连续课程。现修正为
  `gap_width = 0.25 * difficulty`；正式难度 1.0 的 0.25 m 几何保持不变。
- 航点容差从 0.45 m 调整为 0.9 m。课程未规定硬阈值，中心平台半宽约 1 m；
  0.6-1.0 m 敏感性检查表明，放宽只会把已登上中心平台的轨迹计为到达，
  不会把仍在沟外或台阶下的轨迹误判成功。
- H32/H34 的沟槽专家在近障碍对齐起点能稳定到达首航点；有效奖励配置为
  AMP 0.5、导航权重 2、跌倒权重 -5，说明适度 AMP 仍有利于跨越稳定性。
- 台阶课程必须更强地保留 Lab7 能力。H36 只有 AMP 1.0 分支能在 10% 难度
  跨过首个台阶地块并到达航点；降低 AMP 或 gait-preservation 均失败。
- H37-H40 显示 15%-20% 台阶仍是能力断点。提高足端净空目标至 0.12/0.15 m
  未解决问题；12.5% 难度的最好分支推进 7.55 m。下一阶段从 H36 的 10%
  台阶专家按 1.25 个百分点细粒度升阶，并在每一级做近障碍技能验收。
- H41 在 11.25% 台阶上的最佳分支推进 7.57 m，仍未到达中心；10%-11.25%
  附近对不到 0.25 个百分点的高度变化较敏感，后续评测必须使用多 rollout，
  不能用单次结果判断课程是否通过。
- 最低难度跨场景对照中，H17 在 6 个场景平均推进 28.09 m、首航点成功率
  83.3%、平均通过 2.5 个航点；H26 scene31 为 23.72 m、66.7% 和 2.0 个航点。
  这证明 waypoint 导航与多航点切换已经成立，正式难度的主要瓶颈是障碍运动能力。
- 同学 Lab7 Height 提交模型在统一粗糙地形评测中 walking success 为 8.64%，
  高于当前自训温和粗糙模型约 3%；但转换到 `forward_yaw` Course Project 后仅推进
  0.39 m，说明其命令语义/归一化与当前适配不兼容，不能直接替换现有导航底座。

### H42-H43：事件奖励时间尺度与速度课程

- 算法复核发现，`RewardManager` 会把奖励项整体乘以 `dt=0.02`。此前稠密 progress
  已转换为每秒推进率，但一次性的 waypoint 和 route-success 布尔事件没有除以 `dt`，
  导致其积分权重分别从规定的 0.5/5.0 缩小为 0.01/0.1。现由环境适配层把事件转换为
  `event / dt` 脉冲，保持 `student.navigation_reward()` 的规定公式不变，并新增单元测试
  验证积分后的事件奖励恰为 0.5 和 5.0。
- H42 在最低地形难度从 H17 继续训练 4 个场景。四个分支的 6 场景结果均约为
  28.0 m、平均 2.5 个航点、首航点成功率 83.3%，说明时间尺度修复正确但短程微调
  不会凭空提高运动速度。
- 正式路线约 40-80 m，而 5000 步对应 100 s；原 `0.35 m/s` 命令的理论上限只有
  35 m，长路线不可能在时限内完成。H43 因此从 H42 最稳分支训练
  `0.5/0.6/0.8/1.0 m/s` 四档速度。
- 首批 3 场景筛选中，`0.6 m/s` 达到平均推进 42.37 m、路线进度 93.82%、
  首航点成功率 100%、平均 4 个航点、完整路线成功率 66.7%，且跌倒率为 0。
  `0.8/1.0 m/s` 虽同为 66.7% 路线成功，但平均推进约 26.7 m、跌倒率 33.3%，
  因此当前选择 `0.6 m/s` 做 10 起点复核及后续复杂地形迁移。
- H43 `0.6 m/s` 的最低难度 10 起点复核达到平均推进 28.24 m、路线进度
  55.54%、首航点成功率 60%、平均 2.7 个航点、完整路线成功率 30%；跌倒率 40%，
  timeout 率 30%。这锁定了能在时限内完成 Course Project 路线的导航基线。
- H44 在 10% 地形难度比较 AMP 1.0/1.5 和跌倒惩罚 -5/-10。末期模型均发生
  步态遗忘；`-10` 分支 3/3 到首航点后仍全部跌倒，说明增大终止惩罚不能替代
  障碍运动学习。较好的中期 `AMP=1.5, model_50` 推进 20.74 m、平均 2 个航点，
  但跌倒率仍为 100%。
- H45 将 H43 稳定导航模型与 H44 中期障碍模型做 actor 插值。25% 障碍模型比例在
  10% 难度的 3 场景取得 26.51 m、100% 首航点、平均 2.67 个航点和 33.3% 完整
  路线成功，优于 50%/75% 分支。直接迁移至 20% 难度只剩 33.3% 首航点且全部
  跌倒，40% 难度首航点为 0，故后续从 12.5%-15% 继续细粒度课程。
- H45 的 3 场景突破在 6 场景复核中降为 9.89 m、50% 首航点、0 路线成功且全部
  跌倒，说明小样本结果来自场景适配，不能作为泛化突破。H46 对四个单场景短训练的
  actor 参数求均值后再与 H43 融合，最好分支为 16.59 m、50% 首航点、16.7% 路线
  成功，但跌倒率仍为 83.3%；H47 每 10 次更新切换地图也只有 10.16 m 且全部跌倒。
- 根因是原训练结构中 64 个并行环境复制同一张完整地图，训练后融合或串行切图都不能
  让一次 PPO 梯度包含跨地图经验。现新增 `--training-scenes`：把多张完整地图生成为
  terrain columns，按环境均匀分配，并让 WaypointCommand 按 `terrain_types` 选择各自
  路线。4 地图、8 环境 smoke 已通过地图编译、路线分派、奖励计算和一次 PPO 更新；
  actor/critic 维度仍为 285/297，单场景评测与提交路径保持不变。
- H48 首次使用 4 地图同批训练。10% 难度的 `AMP=1.5, lr=1e-6` 分支在 6 个未见
  场景达到 24.70 m、路线进度 47.20%、首航点成功率 66.7%、平均 2.33 个航点、
  路线成功率 16.7%；跌倒率 66.7%、timeout 率 16.7%。它明显优于训练后参数平均的
  16.59 m，证明同一 PPO batch 的场景多样性有效。AMP 1.0 分支为 21.60 m、无路线
  成功；保守 `5e-7` 分支为 16.10 m；直接训练 12.5% 难度仅 8.64 m，故下一档改为
  11.25% 细粒度升阶。
- H49 从 H48 最佳模型继续：11.25% 难度分支在 6 个未见场景达到 17.17 m、
  50% 首航点、平均 1.67 个航点和 16.7% 路线成功，证明课程能在提高难度后保留
  非零完整成功。8 地图/64 env 分支因每图样本减半退化到 11.96 m、0 路线成功；
  更换 4 张训练地图的分支为 16.11 m、66.7% 首航点但全部跌倒。当前正式候选仍为
  H48 的 10% 泛化模型，H49 的 11.25% 模型仅作为 12.5% 升阶起点。
- H50 从 H49 升至 12.5%，比较混合起点、中心起点和 1.5 倍 gait-preservation。
  中心起点最好但仅 11.90 m、66.7% 首航点、平均 1 个航点，路线成功为 0 且最终
  全部跌倒；混合起点与更强步态约束分别退化到 6.39 m 和 6.68 m。由此回退 H49，
  不使用 H50 checkpoint 继续升阶；下一轮应在 11.25%-12.5% 之间缩小步长，并针对
  台阶/沟槽接触失败调整采样或感知，而不是继续增加 gait/终止惩罚。
- H50 后的命令审计提出待验证假设：航点切换到垂直下一航段时，原
  `min_turning_speed=0.1` 会在约 6 s 的 90 度转向过程中继续前进，可能导致机器人
  从中心平台边缘跌落。已将该值贯通为训练/评测/视频/提交共同参数并增加纯函数测试；
  默认值保持 0.1，`0.0` 分支须待 GPU 可用时做零样本与短微调对照后才能作为结论。
- H51 零样本转弯对照已尝试启动，但服务器四张卡均有其他用户进程常驻；新建的
  MuJoCo-Warp 进程持续处于内核 `D` 状态且日志为 0 字节，因此已主动停止本项目进程，
  未把基础设施等待误记为实验失败。GPU 恢复后的第一项工作是用 H49 在 11.25%
  难度分别评测 `min_turning_speed=0.1/0.0`，各 6 个相同 seed；统一比较路线推进、
  完整成功、跌倒目标地形和 `post_waypoint_fall_rate`。只有 `0.0` 在跨场景复核中
  降低航点切换后跌倒且不降低首航点率，才从 H49 启动短程微调。
- 2026-08-14 再次利用 GPU 2 的剩余显存尝试 H51 基准分支时，外部四卡任务在初始化
  窗口重新占满计算单元，本项目进程再次进入 `D` 状态且日志保持 0 字节。已只终止
  本项目 PID 并确认完全退出；这仍记为资源阻塞而非策略失败。后续仅在至少一张卡的
  外部计算任务结束后启动固定的 `0.1/0.0` 同 seed 对照，避免无效进程堆积。
- 提交接口复核确认，课程原始环境在 actor 第 96-97 列输出未缩放的 body-frame
  waypoint 位移，而当前 `forward_yaw` checkpoint 在训练时接收 `[forward_speed,
  yaw_rate]`。提交 `model.py` 中的适配器因此是正式评测所必需的语义转换，并且必须在
  导出策略内部的 observation normalizer 之前执行。新增回归测试直接从导出模板加载
  适配器，验证其数值与训练端 `forward_yaw_command()` 一致、仅修改这两列且不原地
  改写评分环境传入的观测。
- H51 等待期间继续增强评测诊断：结果 JSON 现保留每个 seed 的路线地形序列、推进米数、
  已过航点、终止原因、终止时目标地形与机器人实际所在 tile，并分别汇总按目标和按实际
  位置的跌倒率。这样可区分“朝向某障碍但尚未进入就跌倒”“跨越障碍时跌倒”和“航点
  切换后在中心平台转向跌倒”，后续参数决策不再只依赖 6 个场景的聚合均值。
- 视频接口审计发现，旧 `record_video()` 未接收命令行的地形难度、起点朝向和起点
  偏移，导致阶段 checkpoint 即使按低难度评测，视频仍会默认为满难度 1.0，不能与
  评测指标一一对应。现已贯通上述参数；后续每个视频文件名和报告必须注明 checkpoint、
  scene seed、terrain difficulty、命令速度和单 episode 终止原因。
